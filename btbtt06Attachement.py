...
def getAttachementFromDB(attrFilter, dbUrl,subdir=None):
    ...
    attachementHeaders:[AttachementHeader] = session.query(AttachementHeader) \
        .filter(attrFilter) \
    ...
    for header in attachementHeaders:
        ...
        try:
            ...
            print("Save {} - {} failed with {}".format(header.title,header.link,str(e)))
...
def getAttachementThread(dbUrl, threadLink:str):
    ...
    thread:Thread = session.query(Thread) \
        .filter(Thread.link == threadLink) \
        .limit(FATCH_SIZE).first()
    session.expunge_all()
    session.commit()
